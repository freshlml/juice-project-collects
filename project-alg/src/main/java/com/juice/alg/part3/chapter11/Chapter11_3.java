...
public class Chapter11_3 {
     ...
     *      令 A = s/2^w, 其中 s ∈ (0, 2^w), 则 A ∈ (0, 1) = (0.0...00, 0.1...11)
     *      令 m = 2^p, p ∈ [0, w)                             | w位 |   | w位 |
     *
     *      key * s = key * (A * 2^w) = r1 * 2^w + r0
     *
     *      可以证明 r0 的 p 个最高有效位等于 `⌊2^p * (key*A - ⌊key*A⌋)⌋` 的运算结果:
     *                           |--- a ---|- b -|
     *        令 key * A = a.b = --~----|~~.-----                      // key * A 的结果要不丢失精度的保存，则至少需要 2*w 个有效位的浮点数类型
     *                          |- w位 -|- w位 -|
     *
     *        ⌊2^p * (key*A - ⌊key*A⌋)⌋ = ⌊2^p * 0.b⌋ = b 部分的 p 个最高有效位
     *
     *                                                                  |-- a --|- b -|
     *        key * s = key * (A * 2^w) = (key * A) * 2^w = a.b * 2^w =  ~----~~|-----00.
     *                                                                  |- w位 -|- w位 -|
     *                                                                  |- r1 -|- r0 -|
     *
     *全域散列法
