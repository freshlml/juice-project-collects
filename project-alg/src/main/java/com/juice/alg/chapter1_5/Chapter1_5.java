...
public class Chapter1_5 {
    ...
    /**
     *算法运行时间分析: 最坏情况运行时间(时间复杂度)，平均情况运行时间，期望运行时间
     *
     *   最坏情况运行时间: 最坏情况下运行时间。如插入排序的里层循环，最坏情况是比较j次；如快速排序中partition返回的q位置的最坏情况是0/end-1
     *   平均情况运行时间: 从概率角度计算运行时间。如插入排序的里层循环，从概率角度计算需要比较的次数；如按概率计算快速排序的partition返回的q位置
     *                  a:如何从概率的角度求解
     *                    1).假设输入序列的分布，输入序列的分布决定事件的概率
     *                    2).通常可以抽象出一个随机变量，求解随机变量(可以给出其试验帮助求解)
     *                  b:输入序列的分布
     *                     要求解的量受输入序列的影响，即不同的输入序列导致不同的结果
     *                     则输入序列的分布决定事件的概率: 输入序列本身满足什么样的分布或者合理假设输入序列的分布(如均匀随机的排列)
     *
     *   对于一个算法，可以直接计算得出其最坏情况运行时间
     *   如果输入序列本身满足一定的分布或者可以合理假设输入序列的分布，则可从概率角度求解得出平均情况运行时间
     *   而在实际工程中，更多的情况是 输入序列不满足一定的分布并且不能合理的假设输入序列的分布，则可以根据随机数人为的创造出满足一定分布的输入序列，即期望运行时间
     *
     *   期望运行时间: 通过随机数生成器将输入序列打乱，使打乱后的序列满足一定的分布，从而计算出来的时间
     *              它等于使用同样的输入序列分布的平均情况运行时间
     *
     *   对于一个算法，最坏情况运行时间通常是固定的，但可以优化平均情况运行时间，
     *   可以假设输入序列满足特定的分布，并且在该分布下有比较优秀的平均情况运行时间，
     *   然后使用随机数生成器创造满足该分布的输入序列，就可以得到比较优秀的期望运行时间==需要的比较优秀的平均情况运行时间
     *
     */
