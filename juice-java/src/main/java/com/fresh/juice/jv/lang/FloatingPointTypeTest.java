 ...
 *                    - 二进制存储值层面, 丢掉小数点和小数部分数值，如果数值在 [-2^63, 2^63-1] 范围外，取 Long 的最值返回，否则返回数值
 *
 * 2. long -> double: 当数值∈[-2^53, 2^53]时，数值被完整保存，不会丢失精度
 *                    当数值大于 2^53 或者小于 -2^53 时, 舍入造成数值精度丢失
 *
 *
...
public class FloatingPointTypeTest {
    ...
    //一个小数存储为浮点类型，其整数部分是否能够被完整而正确的保存？
    // 第一、有效位数
    //      小数的整数部分的位数 <= 浮点类型的有效位数
    // 第二、舍入的影响
    //      如 (2 ^ 24 - 1).99999 == 16_777_215.9999 == 1111 1111 1111 1111 1111 1111|.111111...
    //                                                                              舍入
    //      保留 24 个有效位舍入后：1 0000 0000 0000 0000 0000 000|0.    //舍入产生了进位，致使存储值是原小数整数分部分的值 + 1
    //
    //      再如 (2 ^ 24 - 1).0001 == 16_777_215.0001 == 1111 1111 1111 1111 1111 1111|.000000...
    //                                                                              舍入
    //      保留 24 个有效位舍入后：1111 1111 1111 1111 1111 1111|.      //舍入未产生进位，则存储值等于原小数整数部分的值
    //
    //      那么，如果进位发生在小数部分，是否就能一定确保整数部分被完整而正确的保存？
    //      答案是也不一定，因为小数部分如果产生进位，是可能传递到整数部分的
    // 结论：
    //      一个整数部分 ∈ [-2^24, 2^24] 的小数存储为 float，该 float 的整数部分数值要么等于原小数的整数部分，要么是原小数的整数部分加一或减一
