...
public class TypeThreadSafeTest {
    ...
    */
     ...
     *
     * 共享对象的返回值原则
     *    1. 共享对象本身的方法返回值满足"return-type-safe"原则
     *    2. 或者，共享对象对其所属的 scope 之外的返回值满足"return-type-safe"原则
     *    3. 或者，这一原则可能被打破(实现代价太大或者代码编写错误等等)，则要求"任何代码"得到并使用该返回值时不发生线程安全问题，如名义上只读
     *
     *
     * 如果存在 type is thread-safe but not return-type-safe: 任意类型，追踪内部使用的"共享对象"，保证线程安全
     *
     *    1. Type.field 获取"共享对象"。调用方法，其执行是线程安全的。可能需要进入方法内部看看传递的参数的使用是否线程安全。返回值是否满足"return-type-safe"原则
     *
     *    2. Type.method，进入方法内部，追踪"共享对象"(包括参数传递的)，验证其使用是否线程安全。返回值是否满足"return-type-safe"原则
     *
     *    3. 参数、局部构造的对象。进入构造器、方法内部，追踪"共享对象"(包括参数传递的)，验证其使用是否线程安全。返回值是否满足"return-type-safe"原则
     *
     *
     *
     * A thread-safe type
     *    1. 构造器调用，方法调用是线程安全的
     *    2. 方法返回值:
     *       1). 一般的，满足"return-type-safe"原则: 在当前线程中使用(方法调用，返回值等级联的、任意的使用)不存在线程安全问题
     ...
     *
     *    6. Type.method。进入方法内部，追踪"共享对象"(包括参数传递的)，验证其使用是否线程安全。返回值是否满足"return-type-safe"原则
     *
     *    7. 局部变量(包括参数)，变量的读写(数值、引用值)是线程安全的，数值运算是线程安全的
     *       局部变量指向对象。进入构造器、方法内部，追踪"共享对象"(包括参数传递的)，验证其使用是否线程安全。返回值是否满足"return-type-safe"原则
     *
     */
