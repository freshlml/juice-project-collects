 ...
 *    3. 等价代换
 *       1). 加法，减法，乘法运算，根据运算法则进行等价转换. 如 (a + b) * c == a*c + b*c
 *       2). 有除法运算参与的式子等价代换不一定成立，因为存在除不尽带来的值的舍入造成等式两边不再等价:
 *           - 如果等式两边均能够除尽，则两边有相同的正确的结果
 *           - 如果等式一边除不尽，而另一边能够除尽，则可用能够除尽的一边来得到正确的结果
 *           - 如果等式两边均除不尽。两边可能有相同的结果，也可能不同的结果
