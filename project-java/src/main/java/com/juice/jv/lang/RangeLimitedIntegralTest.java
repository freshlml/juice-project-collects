 ...
 *  2. 将 overflow 公式"等价代换"成 no overflow 公式，或者代换成可用 1 处理的公式
 *  3. 使用更长的整数类型或者 BigInteger 来消除 overflow
 *二: 使用位运算特别是移位运算优化运算效率
 *
 *第九: 等价转换
 * 我们通常根据 `+`, `-`, `*`, `/`, `%` 运算的运算法则进行等价代换. 如：
 * - `+`, `-`, `*` 运算等价代换: (a - b) * c == a*c - b*c
 * - `/` 参与的运算，通常需要证明等价代换是否成立，如 (a + b/2) * c == a*c + (b/2)*c，但 (b/2)*c  不一定等于  (b*c)/2 (证明方法，如，看式子两边是否丢失相同精度的值)
 * - `/`, `%` 运算，根据他们的运算法则，对式子等价代换: fresh/.../11_Month
 *
