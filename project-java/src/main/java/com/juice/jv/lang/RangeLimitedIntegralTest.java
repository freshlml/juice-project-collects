 ...
 *     eg:  57  ÷ -12 ==> [ (-4)*(-12) + 9 ] ÷  12  ==>  div = -4, mod =  9            ； 57不能写成 (-5)*(-12) - 3
 *     eg: -57  ÷ -12 ==> [ 4*(-12) + (-9) ] ÷ -12  ==>  div =  4, mod = -9            ；-57不能写成 5*(-12) + 3
 *  floordiv, floormod: 除数、被除数异号，mod 结果不等于0，并且与被除数异号时，产生一个借位。fresh/.../11_Month
 * 2.在数轴上理解 div，mod 运算: |除数| 按 |被除数| 分成了几组，不够一组的数量余下多少 (div, mod 运算不会 overflow 或 underflow)
 *
 *第七: 整数类型转换原理
 *                                    -128    -127      ...        -1            0           1       ...        127
 ...
 *
 *第九: 等价转换
 * 整数 +, -, * 运算，等价转化成立，无论是否 overflow, underflow.
 * div(/), mod(%) 运算，可以根据他们的运算法则，对式子等价代换. fresh/.../11_Month
 *
 */
