...
public class CharTest {
     ...
     *       当码点∈[U+0, U+FFFF]时，一个代码单元即可表示
     *       当码点∈(U+FFFF, U+10FFFF]时，要使用两个代码单元表示
     *
     *     eg  x = U+1D546
     ...
     *      当码点∈[U+0, U+FFFF]时，一个代码单元即可表示，即一个char，获取码点值只需(int) c
     *      当码点∈(U+FFFF, U+10FFFF]时，要使用两个代码单元表示，即两个char，一个char存高位代理，一个char存低位代理，要码点值需要额外计算
     *
     *  char类型不建议使用
    ...
    public static void main(String argv[]) throws Exception {
        ...
        System.out.println(hello.length());         //输出2，𝕆需要两个代码单位存储，一个char只能表示一个代码单元，所以这里String中char[]数组的大小为2,char[0]存储高位代理，char[1]存储低位代理
        ...
        bys = str.getBytes(Charset.forName("gbk"));    //gbk的中文码点和unicode保持一致，gbk在此基础上扩充了中文字符，只看中文，unicode是gbk的子集
