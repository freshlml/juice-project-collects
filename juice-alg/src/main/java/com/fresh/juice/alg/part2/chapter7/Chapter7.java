...
public class Chapter7 {
     ...
     *                                                                              = 1
     *  P(Y<i,j> = 1) = 2/(j-i+1)
     *     如果 i,j 在某次划分中被划分到不同分区，则 i,j 一次比较也不会发生
     *     假定一个有序序列 1 3 5 6 7 15 17 ... n-1 n, i,j 为其任意下标
     *       如果 >j 的位置率先被选为主元进行一次划分后，i,j 仍然在同一分区，同理 <i 的位置率先被选为主元进行一次划分后，i,j 仍然在同一分区
     *       但是，如果 (i, j) 中任意一位置率先被选为主元进行一次划分后，i,j 将位于不同分区，即 i,j 一次比较也不会发生
     *       这样，很容易得出一个结论: 对于任意 i,j，只要 i 位置或 j 位置先于 (i, j) 中所有位置被选为主元，就能保证 i 与 j 比较一次
     *     这一结论可推广至任意顺序的序列
     *
